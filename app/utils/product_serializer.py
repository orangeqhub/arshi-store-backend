...
def serialize_variant(variant) -> dict:
    return {
        ...
        "discount_percentage": calculate_discount(variant.mrp, variant.sale_price),
        "stock_qty": variant.stock_qty,
        "stock_status": get_stock_status(variant.stock_qty, food_labels=True),
        "is_active": variant.is_active,
        "is_default": variant.is_default,
    }


def get_active_variants(product) -> list:
    return sorted(
...
def serialize_store_product_list_item(product) -> dict:
    ...
    default_variant = get_default_variant(product)
    mrp = default_variant.mrp if default_variant else product.mrp
    sale_price = default_variant.sale_price if default_variant else product.sale_price
    stock_qty = default_variant.stock_qty if default_variant else product.stock_qty
    sku = default_variant.sku if default_variant else product.sku
    ...
    return {
        ...
        "discount_percentage": calculate_discount(mrp, sale_price),
        "stock_qty": stock_qty,
        "stock_status": get_stock_status(stock_qty, food_labels=True),
        ...
        "has_variants": bool(get_active_variants(product)),
...
def serialize_store_product_detail(product) -> dict:
    ...
    return {
        ...
        "variants": [serialize_variant(v) for v in active_variants],
        "thumbnail_url": product.thumbnail_url,
