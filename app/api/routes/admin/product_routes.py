...
def _parse_variants(raw: str | None, model):
    """Parses the JSON-encoded `variants` form field.

    Returns None when the field was omitted entirely (create: means "no
    variants", update: means "leave existing variants untouched"), or a
    (possibly empty) list of parsed variant models otherwise.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid variants payload"
        )

    if not isinstance(items, list):
        raise HTTPException(
            status_code=400,
            detail="variants must be a list"
        )
...
async def create_product(
    ...
    payload = ProductCreate(
        ...
        variants=_parse_variants(variants, ProductVariantCreate) or []
    )
...
async def update_product(
    ...
    payload = ProductUpdate(
        ...
        is_bestseller=is_bestseller,
        is_new_arrival=is_new_arrival,
        variants=_parse_variants(variants, ProductVariantUpdate)
    )
